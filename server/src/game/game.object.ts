import {Body, Engine, Runner, Bodies, Composite, Events ,IEventCollision} from 'matter-js'
...
export class GameObject {
    ...
    constructor(
		...
		this.walls["top"] = Bodies.rectangle(640, -250, 1800, 500, { isStatic: true });
		this.walls["bottom"] = Bodies.rectangle(640, 970, 1800, 500, { isStatic: true });
		this.walls["left"] = Bodies.rectangle(-250, 360, 500, 1500, { isStatic: true });
		this.walls["right"] = Bodies.rectangle(1530, 360, 500, 1500, { isStatic: true });
		this.ball = Bodies.circle(640, 360, 10,{inertia: Infinity,
						friction: 0,
						frictionStatic: 0,
						frictionAir: 0,
						restitution: 1});
        ...
        this.bars.push(Bodies.rectangle(50, 300, 10, 100, {inertia: Infinity, friction: 0, frictionStatic: 0, frictionAir: 0,restitution: 1}));
        this.bars.push(Bodies.rectangle(1230, 300, 10, 100, {inertia: Infinity, friction: 0, frictionStatic: 0, frictionAir: 0,restitution: 1}));
        Composite.add(this.engine.world, [this.bars[0], this.bars[1], this.ball, this.walls["top"], this.walls["bottom"], this.walls["left"], this.walls["right"]]);
        // console.log(this.bars);
        this.mov.push(0);
        this.mov.push(0);
        this.score.push(0);
        ...
        const pairs = event.pairs;
			for (let i = 0; i < pairs.length; ++i) {
				...
				if (pair.bodyA === this.ball || pair.bodyB === this.ball) {
					...
					else if (pair.bodyA === this.walls["bottom"] || pair.bodyB === this.walls["bottom"]) {
						console.log("bottom");
						Body.setVelocity(this.ball, {x: this.ball.velocity.x, y: -this.ball.velocity.y});
					}
    ...
    private after_update(){
		...
		this.server.to(this.room).volatile.emit("correction", corr);
	}
}