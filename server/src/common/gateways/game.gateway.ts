...
export class GameGateway {
	...
	async joinGameQueue(
		...
		let matching_opp = this.server.sockets.sockets.get(queue.values().next().value) as CustomSocket;
		...
		const score = await Promise.all([this.userService.updateUser({
			where:{ login: client.user.login },
			data: { current_lobby: game_lobby,
		...
		let gameData: { game_winner_login: string; game_loser_login: string; game_winner_score: number; game_loser_score: number }
		...
		const data = await Promise.all([this.userService.updateUser({
			where:{ login: client.user.login },
			data: { current_lobby: null,
